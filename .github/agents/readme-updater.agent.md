...
Before making any changes, check if `README.md` exists at the project root. If it does not exist, create a new `README.md` file with a top-level heading, followed by the generated `## GitHub Copilot Agents` section as its only content. If it does exist, update only the `## GitHub Copilot Agents` section as described in your instructions.

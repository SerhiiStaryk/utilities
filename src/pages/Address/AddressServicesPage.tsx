...
import { Box, Button, Card, CardContent, Chip, Stack, TextField, Typography, IconButton, Paper, CircularProgress } from '@mui/material';
      ...
      <Card>
        <CardContent>
            ...
            <Typography variant="body2" color="textSecondary" mb={3}>
                Define the default utility services for this address (e.g., Gas, Water, Internet). 
                These will be automatically added when you create a new year.
            </Typography>
            ...
            <Stack direction="row" gap={2} mb={3} alignItems="flex-start">
                ...
                />
                <Button variant="contained" startIcon={<Add />} onClick={handleAddService} disabled={!newServiceName.trim()}>
                    Add
                </Button>
            </Stack>
                ...
                {services.length === 0 ? (
                    ...
                    <Stack direction="row" flexWrap="wrap" gap={1}>
                        {services.map(service => (
                            <Chip 
                                key={service.name} 
                                label={service.accountNumber ? `${service.name} (${service.accountNumber})` : service.name} 
                                onDelete={() => handleRemoveService(service.name)}
                                deleteIcon={<Delete />}
                            />
                        ))}
                    </Stack>
